    ...
    const tools = isAdmin ? [{
      ...
      input_schema: {
        ...
        properties: {
          ...
          due_date: { type: 'string', description: `Due date in YYYY-MM-DD format, or null if no specific date. Use these EXACT dates for relative references — do NOT calculate yourself: today=${(() => { const d = new Date(new Date().toLocaleString('en-US',{timeZone:'America/Vancouver'})); return d.toISOString().split('T')[0]; })()}, tomorrow=${(() => { const d = new Date(new Date().toLocaleString('en-US',{timeZone:'America/Vancouver'})); d.setDate(d.getDate()+1); return d.toISOString().split('T')[0]; })()}, ${(() => { const d = new Date(new Date().toLocaleString('en-US',{timeZone:'America/Vancouver'})); const days = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday']; const results = []; for (let i = 1; i <= 7; i++) { const next = new Date(d); next.setDate(d.getDate()+i); results.push(days[next.getDay()]+'='+next.toISOString().split('T')[0]); } return results.join(', '); })()}` },
